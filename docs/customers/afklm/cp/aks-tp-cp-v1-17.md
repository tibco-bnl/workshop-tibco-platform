...
## Prerequisites
> **_NOTE:_** Documented with TIBCO Platform 1.17.0  
...
export DEFAULT_INGRESS_TLS_SECRET=ingress-cert-secret ## Default TLS Secret name for Ingress 
...
```bash
export CP_PLATFORM_BASE_VERSION=1.17.0 ## Control Plane Base Chart Version
...
export CP_JOB_REPORT_TO_EMAIL="" ## Cron Job reports are sent to this email alias if configured.
export CP_NOTIFICATION_CC_EMAIL="" ## Optional email address to mark as CC for subscription email notifications.
export TP_AUTHORIZED_IP_RANGE="62.250.248.64/32" ## whitlist cidr range to allow access to ingress.
```

## Dataplane base Config
``` bash
export DP_NAMESPACE=platform-integration-dp
```
...
helm upgrade --install --wait --timeout 1h --create-namespace --reuse-values \
  ...
  ${HELM_URL}dp-config-aks --version "1.15.0" -f - <<EOF
clusterIssuer:
  create: false
storageClass:
  azuredisk:
...
### Upgrade Hawk Capability

```bash
helm upgrade --install -n  $CP_NAMESPACE tibco-cp-hawk  ${HELM_URL}tibco-cp-hawk -f $CP_VALUES_FILE  --version='1.17.*'
```
